...
TimeLineGrid::TimeLineGrid(const QSizeF &size, QGraphicsItem *parent) : QGraphicsItem(parent)
{
...
void TaskStorage::removeTask(const quint64& taskId)
{
	...
	if (taskIter != mTasks.end() && (*taskIter) != nullptr)
	{
		...
		int y = taskPtr->getStartTime().date().year();
		int mo = taskPtr->getStartTime().date().month();
		int d = taskPtr->getStartTime().date().day();
		int h = taskPtr->getStartTime().time().hour();
		int m = taskPtr->getStartTime().time().minute();
		int s = taskPtr->getStartTime().time().second();
...
void TimeLineWidget::rearrangeWidgets(QSize size)
{
	...
	QPointF pos = mapToScene(graphicsRect.x(), graphicsRect.y());
